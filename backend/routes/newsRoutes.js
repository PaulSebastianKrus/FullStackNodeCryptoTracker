...
router.get('/', async (req, res) => {
  try {
    ...
    const validCoin = ['all', 'bitcoin', 'ethereum', 'solana', 'ripple', 'dogecoin'].includes(coin) 
      ? coin : 'all';
    ...
    let articles = data.Data.map(article => ({
      ...
      relatedCoins: article.categories.split('|').filter(cat => 
        ['BTC', 'ETH', 'SOL', 'XRP', 'DOGE'].includes(cat)
      )
    }));
