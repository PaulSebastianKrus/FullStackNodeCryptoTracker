    ...
    for (const entry of portfolio) {
      ...
      if (entry.notifyAbove && price > entry.notifyAbove) {
        await sequelize.query(
          ...
          { replacements: { userId, coinId: entry.coinId, threshold: entry.notifyAbove, price } }
        );
      }
      if (entry.notifyBelow && price < entry.notifyBelow) {
        await sequelize.query(
          ...
          { replacements: { userId, coinId: entry.coinId, threshold: entry.notifyBelow, price } }
        );
      }
    }
...
router.delete('/notifications/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await sequelize.query('DELETE FROM "UserNotifications" WHERE "id" = :id', { replacements: { id } });
